...
void MCB::AnimationModel::TwoBoneIkOrder(Vector3D objPos, Vector3D targetPos)
{
   ...
   void Skeleton::calcInterpolatedScaling(Vector3D& Out, float AnimationTime, const NodeAnim* pNodeAnim)
  {
   ...
   void MCB::Skeleton::TwoBoneIK(Node& endJoint, Node& middleJoint)
   {
	   ...
	   float angleFromdefaultTriangle = cosineFrom3LineLength(middleJointBoneLength, rootToEndLength, endJointBoneLength);//余弦定理で角度算出
	   float angleFromTargetTriangle = cosineFrom3LineLength(middleJointBoneLength, localTargetVectorFromRootJoint, endJointBoneLength);//余弦定理で角度算出
