...
namespace MCB
{
    ...
    {
    public:
        PlaneCollider(const Vector3D& normal = { 0,0,0 }, const float& originDistance = 1.0f) 
        {
            normal_ = normal, originDistance_ = originDistance; primitive_ = PrimitiveType::PLANE; 
        };
        void Update() override;
        inline void SetDistance(const float& originDistance) { originDistance_ = originDistance; }
    private:
