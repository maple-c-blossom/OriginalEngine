#pragma once
#include "MCBFloat.h"
namespace MCB
{

	class Vector3D
	{
	public:
		Float3 vec_;
		Vector3D(const Float3& start, const Float3& end);
		Vector3D(const Vector3D& start, const Vector3D& end);
		Vector3D();
		Vector3D(const Float3& vec);
		Vector3D(const float& x, const float& y,const float& z);

		Vector3D V3Get(const Float3& start, const Float3& end);
		float V3Len() const;
		void V3Norm();
		float GetV3Dot(const Vector3D& vector);
		float GetInnerProduct(const Vector3D& vector);
		Vector3D GetV3Cross(const Vector3D& vector);
		Vector3D GetV3Cross(const Vector3D& aVector, const Vector3D& bVector);
		DirectX::XMVECTOR ConvertXMVEC();


		Vector3D GetFrontVec(const Vector3D& RightVec, const Vector3D& UpVec = { 0,1,0 });
		Vector3D GetUpVec(const Vector3D& RightVec, const Vector3D& frontVec = { 0,0,1 });
		Vector3D GetRightVec(const Vector3D& frontVec, const Vector3D& UpVec = { 0,1,0 });

		Vector3D& operator+=(const Vector3D& Vec);
		Vector3D& operator-=(const Vector3D& Vec);
		Vector3D& operator*=(const float& k);
		Vector3D& operator= (const Float3& a);
		Vector3D& operator= (const DirectX::XMVECTOR& a);
	};
	Vector3D operator+(Vector3D& vecA, Vector3D& vecB);
	Vector3D operator-(Vector3D& vecA, Vector3D& vecB);
	Vector3D operator*(Vector3D& vecA,float k);
	Vector3D operator*(const float& k, const Vector3D& vecA);
	Vector3D operator/(Vector3D& vecA,float k);
	Vector3D operator-(Vector3D& vecA);

}

